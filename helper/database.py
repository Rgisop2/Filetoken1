...
class MongoDB:
    ...
    async def store_file_verification_image(self, file_token: str, image_url: str):
        """Store custom verification image for a specific file"""
        await self.user_data.update_one(
            {"_id": f"file_image_{file_token}"},
            {"$set": {"image_url": image_url}},
            upsert=True
        )

    async def get_file_verification_image(self, file_token: str):
        """Get custom verification image for a specific file"""
        doc = await self.user_data.find_one({"_id": f"file_image_{file_token}"})
        return doc.get("image_url") if doc else None

    async def store_batch_verification_image(self, batch_token: str, image_url: str):
        """Store custom verification image for a specific batch"""
        await self.user_data.update_one(
            {"_id": f"batch_image_{batch_token}"},
            {"$set": {"image_url": image_url}},
            upsert=True
        )

    async def get_batch_verification_image(self, batch_token: str):
        """Get custom verification image for a specific batch"""
        doc = await self.user_data.find_one({"_id": f"batch_image_{batch_token}"})
        return doc.get("image_url") if doc else None

    async def remove_file_verification_image(self, file_token: str):
        """Remove custom verification image for a specific file"""
        await self.user_data.delete_one({"_id": f"file_image_{file_token}"})

    async def remove_batch_verification_image(self, batch_token: str):
        """Remove custom verification image for a specific batch"""
        await self.user_data.delete_one({"_id": f"batch_image_{batch_token}"})
